    ...
    ? $('#tor-tbl tbody')
        ...
        .map((_, track) => ({
          ...
          title: $(track)
            ...
            .text(),
          author: $(track)
            .find('td.u-name .u-name a ')
            .text(),
          category: {
            id: $(track)
              .find('td.f-name .f-name a')
              .attr('href')
              .replace(/.*?f=([0-9]*)$/g, '$1'),
            name: $(track)
              .find('td.f-name .f-name a')
          ...
          size: $(track)
            ...
            .text()
            .replace(' ↓', ''),
          torrent: `http://${host}/forum/${track.find('td.tor-size a').attr('href')}`,
          seeds: +$(track)
            .find('b.seedmed')
            .html(),
          leechs: +$(track)
          ...
          uploadDate_string: $(track)
            .find('td:last-child p')
            .text(),
          url: `http://${host}/forum/viewtopic.php?t=${$(track)
            .find('td.t-title .t-title a')
            .attr('data-topic_id')}`,
        }))
        .get()
...
const parsePageInfo = (html) => {
  ...
  const isOnlyOnePage = Boolean(
    $('#main_content_wrap .bottom_info .nav p[style="float: right"]').text()
  );
  if (isOnlyOnePage) {
...
const parseFullInfo = (html) => {
  ...
  const body = $('.post_body')
    .eq(0)
    .ignore('div.sp-wrap')
    .text(); // Parse only first '.post_body', because others are comments, and ignore content in spoller tags.
  const categories = $('.nav.w100.pad_2 a')
    .map((index, a) => $(a).text())
    .get();
  ...
  if (deep) {
    return $(categories)
      .map((_, category) => ({
        ...
        subCategories:
          $(category)
            .find('table.forums tr td:last-child')
            .map((__, subCategory) => ({
              id: $(subCategory)
                .find('h4 a')
                .attr('href')
              ...
              subCategories:
                $(subCategory)
                  .find('.subforums .sf_title a')
                  .map((___, subSubCategory) => ({
                    id: $(subSubCategory)
                      .attr('href')
                      .replace(/(.*)f=([0-9]*)$/g, `$2`),
...
const parseCaptcha = (html) => {
  ...
  return $('table.forumline .mrg_16 img').attr('src')
    ? {
        img: $('table.forumline .mrg_16 img')
          .attr('src')
          .replace('//', ''),
